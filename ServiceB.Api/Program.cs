...
using (var scope = app.Services.CreateScope())
{
    ...
    try
    {
        ...
        if (!context.Database.CanConnect())
        {
            ...
            Console.WriteLine($"Database migration completed successfully for {appName}.");
        }
        else
        {
            Console.WriteLine($"Database already exists, skipping migration for {appName}.");
        }
    }
    catch (Exception ex)
    {
        Console.WriteLine($"Database migration failed for {appName}: {ex.Message}");
        throw;
    }
}
